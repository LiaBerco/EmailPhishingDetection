...
# ---------------------------------------------------------------- title
md(r"""
# Student A â€” Content / NLP Phishing Detector

**Course:** Data Science & Cyber Â· **Part:** Student A (email *content* analysis)
...
legitimate mail), **explainability** (which words drove a decision), and â€” the
main focus â€” **adversarial robustness** (what happens when an attacker obfuscates
...
* **RQ1 â€” Signal.** How well does a TF-IDF + Naive-Bayes / Logistic-Regression
  model separate phishing from legitimate email, on a single confound-free corpus?
* **RQ2 â€” Robustness.** How much does character-level obfuscation degrade the
  detector, and how much do normalisation and character n-grams recover â€” including
  against attacks the defence was *not* designed for?
* **RQ3 â€” Transfer.** Do attacks crafted against the lexical model also hurt
  DistilBERT?
* **RQ4 â€” Trade-off.** What does robustness cost us in interpretability?
...
# ---------------------------------------------------------------- data
md(r"""
## 1. Data â€” loading, cleaning and a realistic base rate
...
confound by construction (we verify the confound directly in Â§3).
...
2. **We rebalance to a legit-majority base rate** (`PHISH_FRACTION = 0.35`). Real
   inboxes are mostly legitimate, and â€” as we will see in Â§13 â€” a phishing-majority
...
rows, and â€” a foreshadowing of the attack section â€” how much non-ASCII text is
...
""")
md(r"""
Next, the most **discriminative** tokens per class â€” a log-odds score, so a word
...
# ---------------------------------------------------------------- features
md(r"""
## 5. From text to numbers â€” TF-IDF features
...
* **word** â€” word 1â€“2 grams (unigrams + bigrams), English stop-words removed;
* **char** â€” character 3â€“5 grams (`char_wb`), which capture sub-word shape and, as
  we will see, are far more robust to obfuscation (`cl1ck` still shares chunks with
  `click`);
* **union** â€” both concatenated.
""")
code(r"""
from sklearn.feature_extraction.text import TfidfVectorizer
...
difference. All three do well on clean text â€” the interesting differences show up
later, under attack.
""")
code(r"""
from sklearn.linear_model import LogisticRegression
...
# ---------------------------------------------------------------- E5 explain
md(r"""
## 11. Explainability and the robustnessâ€“interpretability trade-off (E5)
...
n-grams â€” the features that give robustness later â€” are not human-readable. This is
...
compare it to the default 0.5. We also show the ROC and precisionâ€“recall curves.
""")
code(r"""
...
# ---------------------------------------------------------------- E6/E7 robustness
md(r"""
## 13. Adversarial robustness â€” the main event (E6 / E7)
...
swapped for identical-looking Cyrillic ones â€” `verify` â†’ `vÐµrify`) and **leetspeak**
(`click` â†’ `cl1ck`). Homoglyphs are imperceptible to a human reader, which is what
...
* **character n-grams** â€” already inside the `union` model; robust because `vÐµrify`
  still shares letter-chunks with `verify`;
* **normalisation** â€” fold look-alikes back to ASCII, strip invisible characters,
  de-leet;
...
plt.ylim(0, 1.02); plt.title("Recall collapses under attack â€” char n-grams resist")
plt.legend(); plt.tight_layout(); savefig("F7_budget_sweep"); plt.show()
""")
md(r"""
The full robustness table â€” the centrepiece. Rows are attack/defence settings,
...
""")
code(r"""
# F5 â€” the money figure: clean vs attacked vs defended, both models
...
md(r"""
## 13b. Adversarial training â€” a second, complementary defence
...
(still labelled phishing) and retraining. There is no leakage â€” only *training* rows
...
""")
code(r"""
# F8 â€” the two defences side by side across attack families
...
beats TF-IDF â€” but it is the upgrade path for messier text. We also test **attack
...
usually drops â€” that gap is the real-world cost of distribution shift, and it is the
...
md(r"""
## 16. A smarter attacker â€” importance-guided evasion (extends E6/E7)
...
do than a random one â€” and how few edits does it take to flip a decision?
""")
code(r"""
...
of *random* edits barely moves it. The effort table makes the danger concrete â€” a
...
adaptive attacker â€” same knowledge of the normaliser, but spent intelligently â€” pushes
...
competent, defence-aware adversary is **wider than the uniform attack suggested** â€” the
...
model is **not** confidently wrong on out-of-distribution phishing â€” its scores drift
...
But it does not fully recover â€” best-threshold F1 is still far short of in-corpus â€” and
...
distribution-shift behaviour a deployed detector must be tuned and re-calibrated for â€”
...
The standard deviations are tiny â€” a few thousandths of recall â€” so every headline
...
Normalisation is applied to *every* incoming email, not just attacked ones â€” so we
...
false-positive rate by roughly a factor of three off a very low base â€” a handful of
...
# ---------------------------------------------------------------- F: calibration
md(r"""
## 20. Calibration â€” are the probabilities fit to be fused?
...
In-corpus the model is already well calibrated â€” the reliability curve tracks the
diagonal, the Brier score and ECE are small â€” because its scores are sharply bimodal
...
in-domain mail and **optimistic to feed raw under distribution shift** â€” a reason the
...
md(r"""
## 21. The arms race, part 1 â€” a principled Unicode canonicaliser
...
â€” the same data browsers use to detect spoofed domains (`normalize_skeleton` in
...
The old curated table is essentially useless against the targeted-adaptive attacker â€”
...
byte-identical whether or not the attack was applied â€” then the model literally cannot
...
*class* â€” word insertion, spacing, or paraphrase â€” which is where a rational attacker,
...
md(r"""
## 23. The arms race, part 2 â€” iterative adversarial training
...
retrain, and repeat â€” re-targeting the test attack against each new model so the
...
to shut down the character-obfuscation front â€” one at the input, one in the model.
""")
...
their Latin skeleton, ordinary English mail â€” already ASCII â€” passes through untouched.
...
default threshold â€” even less than on the earlier cross-corpus test, and no better on the
...
code(r"""
import re as _re
CYR = _re.compile(r"[Ð€-Ó¿]"); GRK = _re.compile(r"[Í°-Ï¿á¼€-á¿¿]")
ARM = _re.compile(r"[Ô°-Ö]"); ZW = _re.compile(r"[â€‹-â€â ï»¿Â­]")
def _mixed(t):
    for tok in t.split():
...
The dilution attack (Â§27) works because a linear bag-of-words simply *sums* per-word evidence,
so enough legitimate words out-vote the phishing ones. A defence that does **not** require seeing
the attack: score the **positive evidence present** â€” the sum of the phishing-pushing weights of the
...
plt.title("Evidence-floor recovers dilution recall â€” at a precision cost"); plt.legend()
...
The evidence-floor is **dilution-invariant by construction** â€” filler words carry no phishing weight,
so they cannot lower the score â€” and it is calibrated on *clean* data only (it never sees the attack).
It genuinely helps: recall on heavily-diluted phishing rises from $0.13$ to roughly $0.36$â€“$0.66$ as
...
mitigates it partially, adversarial training (Â§27) removes it but must know the attack, and the durable
...
sentence order) â€” a cheap stand-in for a neural paraphraser â€” and measure recall.
""")
code(r"""
...
rewrites are not an easy evasion â€” the one effective word-level attack remains dilution (Â§27, Â§29).
...
work â€” but the result so far is reassuring, not alarming.
""")
...
core story â€” clean $\rightarrow$ homoglyph attack $\rightarrow$ normalise/skeleton â€” on a **second
...
recovery is strong but not quite to clean â€” but the *conclusion* is corpus-independent: lexical models
...
The modern mixed detector (Â§28, Experiment B) scored high but its two sources are ~99% separable by
provenance â€” a confound. Can we remove it by **stripping mailing-list boilerplate** (quoted replies,
...
Scrubbing the list boilerplate **does not** meaningfully lower the source-separability â€” it stays
...
available. This is exactly why the trustworthy modern result is **Experiment A** (Â§28), which holds the
...
Â§29 mitigated dilution with an **evidence-floor**: take the trained model's weights, clip the
...
Â§21, but covering a completely different attack class (insertion rather than character substitution).
...
HAM_POOL_U = _ham_ranked[:120]     # matches the Â§27 budget range (all budgets <= 120 are real)
...
is that two attack classes are now closed by proof (character confusables, Â§21; insertion, here) and
...
> our 2008-trained model catches only **14%** of modern phishing (Â§26). If **one** employee
...
"detected" at $0.92$--$0.94$. The reason is the genre confound already documented in Â§30/Â§32 --- any
...
specific reported scam. This is a much better operating point than the evidence-floor of Â§29, which
...
UTS-39 skeleton canonicaliser of Â§21 restores similarity and pushes evasion back to a few per cent.
...
the model of Â§28 could realistically be refreshed. These are cheap to measure and they turn the
...
The retrain figure is the one that speaks to Â§28. Refreshing the model on the full training set
...
the Â§21 canonicaliser on every message first. None of that is included here, and the URL-stripping
design (Â§3) means this detector is only ever half of a deployed system --- the partner detector
...
The rule-based paraphrase of Â§30 did not evade the model, but it only swapped known synonyms and
...
explicitly exclude: Â§21 proves nothing can be gained by changing *letters*, Â§32b proves nothing by
...
# score with the working union model and the monotone certified model (Â§32b)
def _Ur(txts):
...
neural rewrite, which confirms and strengthens Â§30: the model's redundant word *and* character
...
code(r"""
print("="*64)
print("STUDENT A â€” CONTENT DETECTOR: HEADLINE RESULTS")
print("="*64)
