...
"phishing share is a **knob, not a fact of nature** â€” so the headline metrics should always be read "
...
("F2_discriminative", H +
"The class split is uneven but not extreme â€” the minority class is still well represented â€” so ordinary "
...
"almost entirely plain ASCII, so an email suddenly full of exotic look-alike characters is anomalous â€” "
...
"baseline â€” i.e. the corpora are essentially separable on writing style alone. The lesson is blunt: had "
...
"know **vocabulary**, character features know **shape**. On clean text this distinction barely matters â€” "
...
"catching **zero** phishing â€” a detector that detects nothing, with recall and MCC pinned at the floor. "
...
"close to) the confidence intervals â€” so we should **not** over-read which one is 'best' by a thousandth "
...
"Complement NB edges plain Multinomial NB â€” the direction the literature predicts, which we probe "
...
"Complement NB is not a fixed bonus â€” it *grows with the imbalance*, exactly the mechanism the original "
...
("best C =", H +
"The setting chosen on validation transfers to the test set with almost no gap between the two â€” the "
...
"shows the tokens that carry the most pull overall â€” urgency, credentials and money on the phishing "
...
"the model's log-odds â€” so this is the model's own arithmetic, not a keyword guess or a LIME-style "
...
"get â€” **robustness and interpretability pull in opposite directions**."),

("T4_threshold", H +
"Because the model is already near-perfect, moving the threshold barely changes the outcome â€” but the "
...
"homoglyph attack, the **word-only** model loses a large chunk of its recall â€” on the order of a quarter "
...
"Two defences answer it, in different ways. **Character n-grams** never fall far in the first place â€” the "
"union model stays high throughout â€” because sub-word chunks survive letter swaps. **Normalisation** "
...
"family the defence was never tuned for, normalisation *still* recovers the model â€” not because we "
...
"risk, but a defence-aware attacker always keeps a residual foothold â€” **no silver bullet**, only a "
"raised cost of attack."),

("F8_adversarial_training", H +
"Two defences, two different failure modes â€” and they line up to cover each other. **Adversarial "
...
"idea of obfuscation. **Normalisation** is the mirror image â€” it was not tuned for that held-out family "
...
("T7_distilbert", H +
"Two things stand out. First, the transformer does **not** beat the tuned TF-IDF model on clean text â€” "
...
"security project, the homoglyph attack barely transfers to DistilBERT â€” its recall on phishing stays "
...
"a differently-tokenised model â€” a concrete example of **limited attack transferability**."),

("T6_cross_corpus", H +
"This is the sobering number and the most important caveat in the project. Everything above is "
"*in-corpus* â€” train and test drawn from the same source. The moment we test on genuinely different "
